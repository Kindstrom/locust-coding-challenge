...
from app.models import CPUUsage, TestRun, User
...
def get_cpu_usage(test_run_id: uuid.UUID,user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    test_run = session.exec(select(TestRun).where(TestRun.id == test_run_id, TestRun.user_id == user.id)).first()
    if not test_run:
        raise HTTPException(status_code=404, detail="Test run not found or does not belong to the user")

    cpu_usages = session.exec(select(CPUUsage).where(CPUUsage.test_run_id == test_run_id)).all()
...
def record_cpu_usage(
    ...
    cpu_usage = CPUUsage(cpu_percent=cpu_percent, test_run_id=test_run.id)
    session.add(cpu_usage)
    session.commit()

    return {"message": "CPU usage recorded successfully", "cpu_percent": cpu_percent}
